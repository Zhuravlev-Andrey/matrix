...
START_TEST(create_4) {
  ...
  const int columns = 2;
  matrix_t m = {0};
  ck_assert_int_eq(s21_create_matrix(rows, columns, &m), INVALID_MATRIX);
}
END_TEST
START_TEST(create_null_result) {
  const int rows = 1;
  const int columns = 2;
  ck_assert_int_eq(s21_create_matrix(rows, columns, NULL), INVALID_MATRIX);
}
END_TEST
...
START_TEST(sub_matrix_3) {
  ...
  s21_remove_matrix(&matrix2);
  return_value = s21_sub_matrix(&matrix1, &matrix2, &matrix_result);
  ck_assert_int_eq(return_value, INVALID_MATRIX);
}
END_TEST
...
START_TEST(determinant15) {
  ...
  double res = 0;
  int ret = s21_determinant(&A, &res);
  ck_assert_int_eq(CALC_ERROR, ret);
  s21_remove_matrix(&A);
}
END_TEST
...
START_TEST(determinant16) {
  ...
  double res = 0;
  int ret = s21_determinant(&A, &res);
  ck_assert_int_eq(INVALID_MATRIX, ret);
  s21_remove_matrix(&A);
}
END_TEST
